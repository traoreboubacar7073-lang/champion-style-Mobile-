...
class _DevisScreenState extends State<DevisScreen> {
  ...
  void _openAdd() async {
    await showAppBottomSheet(
      context,
      title: 'Nouveau devis',
      child: _DevisForm(clients: _clients, onSaved: () { Navigator.of(context).pop(); _load(); }),
    );
  }
...
class _DevisForm extends StatefulWidget {
  ...
  const _DevisForm({required this.clients, required this.onSaved});
...
class _DevisDetail extends StatelessWidget {
  final Devis devis;
  final VoidCallback onConvertCommande;
  final VoidCallback onConvertFacture;
  final VoidCallback onDelete;
  const _DevisDetail({required this.devis, required this.onConvertCommande, required this.onConvertFacture, required this.onDelete});
    ...
    return Column(
      ...
      children: [
        ...
        if (devis.statut == 'Accepté') ...[
          GoldButton(label: 'Convertir en commande', onPressed: onConvertCommande),
          const SizedBox(height: 10),
          GhostButton(label: 'Convertir en facture', onPressed: onConvertFacture),
          const SizedBox(height: 14),
        ],
        TextButton(
          onPressed: onDelete,
